...
// Prepara y garantiza la carpeta destino para una actividad existente
export async function prepareActivityUpload(req, res, next) {
  try {
    const activityId = Number(req.params.id || req.body.activityId);
    if (!activityId) {
      return res.status(400).json({ error: "Falta el parámetro activityId o :id en la ruta" });
    }
    ...
    const prefix = activity.title.toLowerCase().replace(/\s+/g, "").slice(0, 5);
    ...
    req.activityUploadPath = fullPath;
    req.activityFolderName = folderName;
    next();
  } catch (err) {
    console.error("Error en prepareActivityUpload:", err);
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = req.activityUploadPath || path.join("uploads", "activities", "experiences", "tmp");
    try {
...
export const uploadActivityMedia = (req, res, next) => {
